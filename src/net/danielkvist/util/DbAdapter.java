...
/**
 * This class handles all the database IO together with creating and updating
 * tables and the database when necessary. It also is responsible for adding the
 * default data in the settings and accounts tables.
 * 
 * @author Daniel Kvist
 * 
 */
public class DbAdapter {
	...
	private static final String DATABASE_INIT_SETTING_SUM = "INSERT INTO "
			+ DATABASE_TABLE_SETTINGS + " (" + KEY_NAME + ","
			+ KEY_SETTING_VALUE + ") " + "values " + "('"
			+ Setting.SETTING_FIELD_SUM + "',0" + ");";

	private static final String DATABASE_INIT_SETTING_TAX = "INSERT INTO "
			+ DATABASE_TABLE_SETTINGS + " (" + KEY_NAME + ","
			+ KEY_SETTING_VALUE + ") " + "values " + "('"
			+ Setting.SETTING_FIELD_TAX + "',0" + ");";

	private static final String DATABASE_INIT_SETTING_COMMENT = "INSERT INTO "
			+ DATABASE_TABLE_SETTINGS + " (" + KEY_NAME + ","
			+ KEY_SETTING_VALUE + ") " + "values " + "('"
			+ Setting.SETTING_FIELD_COMMENT + "',0" + ");";

	private static final String DATABASE_INIT_SETTING_LOCATION = "INSERT INTO "
			+ DATABASE_TABLE_SETTINGS + " (" + KEY_NAME + ","
			+ KEY_SETTING_VALUE + ") " + "values " + "('"
			+ Setting.SETTING_FIELD_LOCATION + "',0" + ");";

	private static final String DATABASE_INIT_SETTING_ACCOUNT = "INSERT INTO "
			+ DATABASE_TABLE_SETTINGS + " (" + KEY_NAME + ","
			+ KEY_SETTING_VALUE + ") " + "values " + "('"
			+ Setting.SETTING_FIELD_ACCOUNT + "',0" + ");";

	private static final String DATABASE_INIT_SETTING_DEFAULT_ACCOUNTS = "INSERT INTO "
			+ DATABASE_TABLE_SETTINGS
			+ " ("
			+ KEY_NAME
			+ ","
			...
			+ "values "
			+ "('"
			+ Setting.SETTING_ACCOUNT_DEFAULTS + "',0" + ");";

	private static final String DATABASE_INIT_SETTING_STORAGE = "INSERT INTO "
			+ DATABASE_TABLE_SETTINGS + " (" + KEY_NAME + ","
			+ KEY_SETTING_VALUE + ") " + "values " + "('"
			+ Setting.SETTING_STORAGE + "',0" + ");";

	private static final String DATABASE_INIT_SETTING_SOUND = "INSERT INTO "
			+ DATABASE_TABLE_SETTINGS + " (" + KEY_NAME + ","
			+ KEY_SETTING_VALUE + ") " + "values " + "('"
			+ Setting.SETTING_SOUND + "',0" + ");";
	 ...
	 */
	public long createReceipt(String name, String photo, long timestamp,
			String locLat, String locLong, String sum, String tax,
			String comment, long account_id) {
		ContentValues values = putReceiptValues(name, photo, timestamp, locLat,
				locLong, sum, tax, comment, account_id);
		return db.insert(DATABASE_TABLE_RECEIPTS, null, values);
	}

	/**
	 * Creates a new row in the accounts table with the receipt account
	...
	/**
	 * Deletes the item whose rowId corresponds with the parameter. Make sure to
	 * set which table to delete from before calling this.
	 * 
	 * @param rowId
	 *            id to delete
	...
	public boolean deleteItem(long rowId) {
		if (SELECTED_TABLE.equals(""))
			throw new IllegalAccessError(
					"Unknown database to delete row from, please select a table.");
	...
	public boolean deleteReceiptAccount(ReceiptAccount receiptAccount) {
		...
		db.update(DATABASE_TABLE_RECEIPTS, values, KEY_ACCOUNT_ID + "="
				+ receiptAccount.getCode(), null);
		return deleteItem(receiptAccount.getRowId());
	}

	/**
	 * Fetches all receipt accounts from the database and returns a Cursor
	 * pointing to the first row.
	 * 
	 * @return Cursor or null
	 */
	public Cursor fetchReceiptAccounts() {
		return db.query(DATABASE_TABLE_ACCOUNTS, new String[] { KEY_ROWID,
				KEY_NAME, KEY_CODE, KEY_CATEGORY }, null, null, null, null,
				KEY_CODE);
	}
	...
	public Cursor fetchReceipts(int limit) {
		...
		if (limit > 0) {
			cursor = db.query(DATABASE_TABLE_RECEIPTS, new String[] {
					KEY_ROWID, KEY_NAME, KEY_PHOTO, KEY_TIMESTAMP,
					KEY_LOCATION_LAT, KEY_LOCATION_LONG, KEY_SUM, KEY_TAX,
					KEY_COMMENT, KEY_ACCOUNT_ID }, null, null, null, null,
					KEY_TIMESTAMP + " DESC," + KEY_TIMESTAMP + " DESC",
					String.valueOf(limit));
		} else {
			cursor = db
					.query(DATABASE_TABLE_RECEIPTS, new String[] { KEY_ROWID,
							KEY_NAME, KEY_PHOTO, KEY_TIMESTAMP,
							KEY_LOCATION_LAT, KEY_LOCATION_LONG, KEY_SUM,
							KEY_TAX, KEY_COMMENT, KEY_ACCOUNT_ID }, null, null,
							null, null, null);
		}
		return cursor;
	}
	...
	public Cursor fetchAllSettings() {
		return db.query(DATABASE_TABLE_SETTINGS, new String[] { KEY_ROWID,
				KEY_NAME, KEY_SETTING_VALUE }, null, null, null, null, null);
	}
	...
	public Cursor fetchReceipt(long rowId) {
		...
		return db.query(true, DATABASE_TABLE_RECEIPTS, new String[] {
				KEY_ROWID, KEY_NAME, KEY_PHOTO, KEY_TIMESTAMP,
				KEY_LOCATION_LAT, KEY_LOCATION_LONG, KEY_SUM, KEY_TAX,
				KEY_COMMENT, KEY_ACCOUNT_ID }, KEY_ROWID + "=" + rowId, null,
				null, null, null, null);

	}

	/**
	 * Return a Cursor pointing to the receipts that matches the time span
	 * passed in.
	 * 
	 * @param timeFrom
	...
	public Cursor fetchReceipts(long timeFrom, long timeTo) {
		return db.query(true, DATABASE_TABLE_RECEIPTS, new String[] {
				KEY_ROWID, KEY_NAME, KEY_PHOTO, KEY_TIMESTAMP,
				KEY_LOCATION_LAT, KEY_LOCATION_LONG, KEY_SUM, KEY_TAX,
				KEY_COMMENT, KEY_ACCOUNT_ID }, KEY_TIMESTAMP + ">" + timeFrom
				+ " AND " + KEY_TIMESTAMP + "<" + timeTo, null, null, null,
				KEY_TIMESTAMP + " DESC", null);
	}

	/**
	 * Return the sum of all the sum fields that matches the receipt account
	 * code.
	 * 
	 * @param codeWhereArgs
	 *            the receipt account code where arguments on the form: 3000 OR
	 *            4000 OR 234...
	 * @return Cursor or null
	 */
	public Cursor fetchReceiptsSum(String codeWhereArgs) {
		return db.rawQuery("SELECT SUM(" + KEY_SUM + ") FROM "
				+ DATABASE_TABLE_RECEIPTS + " WHERE " + KEY_ACCOUNT_ID + "="
				+ codeWhereArgs, null);
	}
	...
	public Cursor fetchSetting(String name) {
		...
		return db.query(true, DATABASE_TABLE_SETTINGS, new String[] {
				KEY_ROWID, KEY_NAME, KEY_SETTING_VALUE }, KEY_NAME + "='"
	...
	public Cursor searchReceiptName(String query) {
		return db.query(DATABASE_TABLE_RECEIPTS, new String[] { KEY_ROWID,
				KEY_NAME, KEY_PHOTO, KEY_TIMESTAMP, KEY_LOCATION_LAT,
				KEY_LOCATION_LONG, KEY_SUM, KEY_TAX, KEY_COMMENT,
				KEY_ACCOUNT_ID }, KEY_NAME + " LIKE ?", new String[] { "%"
	...
	public boolean updateReceiptAccount(long rowId, long code, String name,
		...
		values.put(KEY_NAME, name);
		values.put(KEY_CATEGORY, category);
		return db.update(DATABASE_TABLE_ACCOUNTS, values, KEY_ROWID + "="
				+ rowId, null) > 0;
	}

	/**
	 * Updates the corresponding row in the database with the information passed
	 * in.
	 * 
	 * @param rowId
	 ...
	 */
	public int updateReceipt(long rowId, String name, String photo,
			long timestamp, String locLat, String locLong, String sum,
			String tax, String comment, long account_id) {
		ContentValues values = putReceiptValues(name, photo, timestamp, locLat,
				locLong, sum, tax, comment, account_id);
		return db.update(DATABASE_TABLE_RECEIPTS, values, KEY_ROWID + "="
				+ rowId, null);
	}
	...
	public boolean updateSetting(String name, int value) {
		...
		return db.update(DATABASE_TABLE_SETTINGS, values, KEY_NAME + "='"
				+ name + "'", null) > 0;
	}
	...
	public Cursor fetchReceiptAccountCategories() {
		return db.query(true, DATABASE_TABLE_ACCOUNTS,
				new String[] { KEY_CATEGORY }, null, null, null, null,
				KEY_CATEGORY + " ASC", null);
	}

	/**
	 * Saves the values in a ContentValues object that is then return to the
	 * caller
	 * 
	 * @param name
	 * @param photo
	 ...
	 */
	private ContentValues putReceiptValues(String name, String photo,
			long timestamp, String locLat, String locLong, String sum,
			String tax, String comment, long account_id) {
		ContentValues values = new ContentValues();
		values.put(KEY_NAME, name);
	...
	/**
	 * Inner class that handles first initialization of data in the database and
	 * creates its tables.
	 */
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		/**
		 * Creates tables, initiates setting values and imports the accounts.sql
		 * file by using a dbTransaction
		 */
		@Override
		public void onCreate(SQLiteDatabase db) {
			String[] queries = { DATABASE_CREATE_TABLE_RECEIPTS,
					DATABASE_CREATE_TABLE_SETTINGS, DATABASE_INIT_SETTING_SUM,
					DATABASE_INIT_SETTING_TAX, DATABASE_INIT_SETTING_COMMENT,
					DATABASE_INIT_SETTING_LOCATION,
					DATABASE_INIT_SETTING_ACCOUNT,
					DATABASE_INIT_SETTING_DEFAULT_ACCOUNTS,
					DATABASE_INIT_SETTING_STORAGE, DATABASE_INIT_SETTING_SOUND };
		...
		private void initDatabaseData(SQLiteDatabase db) {
			...
			try {
				...
				br = new BufferedReader(new InputStreamReader(context
						.getAssets().open("accounts.sql")), 1024 * 4);
				String line = null;
				db.beginTransaction();
				...
				Log.e("SQLite", "read database init file error");
			} finally {
				db.endTransaction();
				if (br != null) {
					try {
						...
						Log.e("SQLite", "buffer reader close error");
					}
				}
			}
		}
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			Log.w(TAG, "Upgrading " + DATABASE_TABLE_RECEIPTS + " and "
