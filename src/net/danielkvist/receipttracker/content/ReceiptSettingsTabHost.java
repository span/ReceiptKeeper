...
/**
 * This is a custom TabHost that uses an animated onTabChangeListener and also
 * handles the showing/hiding of the menu items in it's parent when switching
 * between the tabs.
 * 
 * @author Daniel Kvist
 * 
 */
public class ReceiptSettingsTabHost extends TabHost implements
	...
	/**
	 * Sets the references of the option items that are to be switched when tabs
	 * change
	 * 
	 * @param deleteItem
	 *            the deleteItem
	 ...
	 *            the addItem
	 */
	public void setOptionItems(MenuItem deleteItem, MenuItem saveItem,
			MenuItem addItem) {
	...
	/**
	 * Overrides the parents behaviour after first calling the super method. It
	 * then sets up this specific TabHost and TabSpec to use the tags and
	 * listeners needed for this custom TabHost.
	 */
	@Override
	public void setup() {
		super.setup();
	...
	/**
	 * Sets up the basic views and controls that the user can see and interact
	 * with on the screen.
	 */
	private void setupViews() {
		...
		categoryAdapter = new ReceiptAccountCategoryAdapter(context,
				android.R.layout.simple_spinner_item, categoryList);
		categorySpinner = (Spinner) findViewById(R.id.category_spinner);
		categorySpinner.setAdapter(categoryAdapter);

		Switch turnoffSwitch = (Switch) findViewById(R.id.turnoff);
		turnoffSwitch.setChecked(settingsMap
				.get(Setting.SETTING_ACCOUNT_DEFAULTS) == View.VISIBLE);
		turnoffSwitch.setOnCheckedChangeListener(this);

	}

	/**
	 * Listener for the switches that saves each Setting as needed when they
	 * change.
	 */
	@Override
	...
	/**
	 * When an item in the Spinner has been selected we want to update the
	...
	/**
	 * Helper method that gets the current selected position of the Spinner and
	 * sets the edit text views accordingly. If the code is not user provided we
	...
	public void updateFields() {
		...
		if (position >= receiptAccounts.size()) {
			// If we remove the last item in the list, it will be out of bounds
			// when we update the fields
		...
		if (!receiptAccount.isUserAdded()) {
			...
			displayName = getResources().getString(
					getResources().getIdentifier(name, "string",
							"net.danielkvist.receipttracker"));
		} else {
