...
public class MyContentProvider extends ContentProvider {
	...
	private static final String RECEIPT_PATH = BASE_PATH + "/"
			+ ReceiptTable.TABLE_NAME;
	private static final String ACCOUNT_PATH = BASE_PATH + "/"
			+ AccountTable.TABLE_NAME;
	private static final String SETTING_PATH = BASE_PATH + "/"
			+ SettingTable.TABLE_NAME;

	public static final Uri CONTENT_URI_RECEIPT = Uri.parse("content://"
			+ AUTHORITY + "/" + RECEIPT_PATH);
	public static final Uri CONTENT_URI_ACCOUNT = Uri.parse("content://"
			+ AUTHORITY + "/" + ACCOUNT_PATH);
	public static final Uri CONTENT_URI_SETTING = Uri.parse("content://"
			+ AUTHORITY + "/" + SETTING_PATH);

	public static final String CONTENT_TYPE_RECEIPT = ContentResolver.CURSOR_DIR_BASE_TYPE
			+ "/receipts";
	public static final String CONTENT_ITEM_TYPE_RECEIPT = ContentResolver.CURSOR_ITEM_BASE_TYPE
			+ "/receipt";
	...
	public static final String CONTENT_TYPE_SETTING = ContentResolver.CURSOR_DIR_BASE_TYPE
			+ "/settings";
	public static final String CONTENT_ITEM_TYPE_SETTING = ContentResolver.CURSOR_ITEM_BASE_TYPE
			+ "/setting";
	...
	synchronized public int delete(Uri uri, String selection,
		...
		switch (uriType) {
		case RECEIPTS:
			rowsDeleted = sqlDB.delete(ReceiptTable.TABLE_NAME, selection,
					selectionArgs);
			break;
		case RECEIPT_ID:
			if (TextUtils.isEmpty(selection)) {
				rowsDeleted = sqlDB.delete(ReceiptTable.TABLE_NAME,
						ReceiptTable.KEY_ROWID + "=" + id, null);
			} else {
				rowsDeleted = sqlDB
						.delete(ReceiptTable.TABLE_NAME, ReceiptTable.KEY_ROWID
								+ "=" + id + " and " + selection, selectionArgs);
			}
			break;
		case ACCOUNTS:
			rowsDeleted = sqlDB.delete(AccountTable.TABLE_NAME, selection,
					selectionArgs);
			break;
		case ACCOUNT_ID:
			if (TextUtils.isEmpty(selection)) {
				rowsDeleted = sqlDB.delete(AccountTable.TABLE_NAME,
						AccountTable.KEY_ROWID + "=" + id, null);
			} else {
				rowsDeleted = sqlDB
						.delete(AccountTable.TABLE_NAME, AccountTable.KEY_ROWID
								+ "=" + id + " and " + selection, selectionArgs);
			}
			break;
		case SETTINGS:
			rowsDeleted = sqlDB.delete(SettingTable.TABLE_NAME, selection,
					selectionArgs);
			break;
		case SETTING_ID:
			if (TextUtils.isEmpty(selection)) {
				rowsDeleted = sqlDB.delete(SettingTable.TABLE_NAME,
						SettingTable.KEY_ROWID + "=" + id, null);
			} else {
				rowsDeleted = sqlDB
						.delete(SettingTable.TABLE_NAME, SettingTable.KEY_ROWID
								+ "=" + id + " and " + selection, selectionArgs);
			}
			break;
		default:
	...
	synchronized public Uri insert(Uri uri, ContentValues values) {
		...
		getContext().getContentResolver().notifyChange(uri, null); // FIXME
																	// should
																	// this be
																	// returnUri?
		return returnUri;
	...
	synchronized public int update(Uri uri, ContentValues values,
		...
		switch (uriType) {
		case RECEIPTS:
			rowsUpdated = sqlDB.update(ReceiptTable.TABLE_NAME, values,
		...
		case RECEIPT_ID:
			if (TextUtils.isEmpty(selection)) {
				...
				rowsUpdated = sqlDB
						.update(ReceiptTable.TABLE_NAME, values,
								ReceiptTable.KEY_ROWID + "=" + id + " and "
										+ selection, selectionArgs);
			}
			break;
		case ACCOUNTS:
			rowsUpdated = sqlDB.update(AccountTable.TABLE_NAME, values,
					selection, selectionArgs);
			break;
		case ACCOUNT_ID:
			if (TextUtils.isEmpty(selection)) {
				...
				rowsUpdated = sqlDB
						.update(AccountTable.TABLE_NAME, values,
								AccountTable.KEY_ROWID + "=" + id + " and "
										+ selection, selectionArgs);
			}
			break;
		case SETTINGS:
			rowsUpdated = sqlDB.update(SettingTable.TABLE_NAME, values,
					selection, selectionArgs);
			break;
		case SETTING_ID:
			if (TextUtils.isEmpty(selection)) {
				...
				rowsUpdated = sqlDB
						.update(SettingTable.TABLE_NAME, values,
								SettingTable.KEY_ROWID + "=" + id + " and "
										+ selection, selectionArgs);
			}
			break;
		default:
